package com.videojs.vpaid {
    ...
    public class AdContainer extends Sprite {
        ...
        private var _idleTimer:Timer = new Timer(1000, 3);
        private var _lastAdVolumne:Number;

        private var _isfirstPlay:Boolean = false;
        private var _isAdLoaded:Boolean = false;
        private var _ishasRemainingTime:Boolean = false;
        ...
        public function get time(): Number {
            if (_model.duration > 0 &&
               ...
               //ExternalInterface.call("console.info", "vpaidcontainer", '_model.duration/_vpaidAd.adRemainingTime:' + _model.duration + '/' + _vpaidAd.adRemainingTime);
                if (_vpaidAd.adRemainingTime > 0) {
                    _ishasRemainingTime = true;
                }

                if (!_ishasRemainingTime) {
                    return 0;
                }
        ...
        public function resize(width: Number, height: Number, viewMode: String = "normal"): void {
           //ExternalInterface.call("console.info", "vpaidcontainer", 'resize');
            if (hasActiveAdAsset) {
        ...
        private function onAdLoaded(evt: Object): void {
            ...
            try {
             ...
             //ExternalInterface.call("console.info", "vpaidcontainer", 'AdLoaded');
            _model.broadcastEventExternally(ExternalEventName.ON_VAST_LOADED);
        ...
        public function startAd(): void {
            ...
            _isfirstPlay = true;
            //_model.broadcastEventExternally(ExternalEventName.ON_RESUME);
            //_model.broadcastEventExternally(ExternalEventName.ON_WAITING);
            if (_isAdLoaded) {
                _ackTimer.reset();
                _ackTimer.start();
        ...
        private function onIdleCheck(evt: Object): void {
            if (!_vpaidAd) {
                //ExternalInterface.call("console.warn", "vpaidcontainer", 'vpaidAd check: ' + _idleTimer.currentCount + '/' + 2);
                if (_idleTimer.currentCount <= 1) return;
        ...
        private function successfulCreativeLoad(evt: Object): void {
           ...
           //ExternalInterface.call("console.info", "vpaidcontainer", "handshakeVersion", ver);     
            if (ver.indexOf("2.") == 0) {
                _model.broadcastEventExternally(ExternalEventName.ON_VAST_HANDSHAKE_VERSION, ver);
            }
            /*if (ver.indexOf("1.") == 0) {
                try {
            ...
            try {
               ...
               //ExternalInterface.call("console.info", "vpaidcontainer", "initAd", String(_model.bitrate));
                ...
                //_model.stageRect.width, _model.stageRect.height
